...
```
Claude Code event ──stdin(JSON)──▶ nc -U ~/.qdf-hook/d.sock ──▶ qdf-hookd ──▶ nc stdout ──▶ Claude Code
                                                                     │
...
The installed hook command is generated by `hookCommand`/`ncArgs` in
`cmd/qdf-hook/init.go`: `nc <flags> -U <sock> 2>/dev/null || <exe> <sub>`. The
flags differ by platform because `-N` (mandatory: it half-closes the write
side so the daemon's `io.ReadAll`-to-EOF unblocks) is a real flag on
Linux/BSD's OpenBSD `nc` but absent on macOS's `/usr/bin/nc`, which already
half-closes on stdin EOF and would error out (falling correctly, if slowly, to
the CLI) if passed `-N`. `QDF_NC_ARGS` overrides the auto-detected flags for a
nonstandard `nc`.
